...
if __name__ == "__main__":
    ...
    if args.mode=='test':
        ...
        np.save(f'sims_strong/sim_data_alpha_{prior_names[0]}.npy', data)
        np.save(f'sims_strong/sim_snapshots_alpha_{prior_names[0]}.npy', snapshots)
        np.save(f'sims_strong/market_id_data_{prior_names[0]}.npy', market_histories)
    else:
        ...
        for i in range(len(result_data)):
            ...
            np.save(f'sims_strong/sim_data_alpha_{prior_names[i]}.npy', data)
            np.save(f'sims_strong/sim_snapshots_alpha_{prior_names[i]}.npy', snapshots)
            np.save(f'sims_strong/market_id_data_{prior_names[i]}.npy', market_histories)
    
    
    print(f'saved results for prior values a={PRIOR_A}, b={PRIOR_B} to sims_strong folder.')
